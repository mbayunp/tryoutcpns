...
export default function PackageDetail() {
  ...
  const { packages, user, fetchPackages } = useExamStore();
  ...
  if (!pkg) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#8C0C14]"></div>
  ...
  const handleBuyNow = () => {
    ...
    Swal.fire({
      ...
      html: `
        <div class="text-left space-y-3 font-body-md text-slate-700">
          ...
          <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2">
            ...
            <p class="font-mono text-lg text-[#8C0C14] font-bold">137-00-213964-2</p>
            <p class="text-xs text-slate-400 font-bold">a.n. WILDAN CASN</p>
            <p class="font-bold text-slate-850 mt-2 text-sm">Jumlah Transfer: <span class="text-[#8C0C14]">${formatRupiah(pkg.price)}</span></p>
          </div>
      ...
      if (result.isConfirmed) {
        const message = `Halo Admin, saya ingin konfirmasi pembayaran.%0A%0ANama: ${user?.name || 'User'}%0AEmail: ${user?.email || 'email'}%0APaket: ${pkg.title}%0A%0ASaya telah melakukan transfer sebesar ${formatRupiah(pkg.price)}. Berikut bukti transfernya:`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-4">
              <span className="bg-[#8C0C14]/10 text-[#8C0C14] font-bold text-[10px] uppercase tracking-wider px-3 py-1 rounded-full">
                {pkg.category}
              </span>
              ...
              <div className="flex gap-6 text-xs font-bold text-slate-500 pt-4 border-t border-slate-50">
                <span className="flex items-center gap-1.5"><Clock className="h-4 w-4 text-[#8C0C14]" /> {pkg.duration} Menit Pengerjaan</span>
                <span className="flex items-center gap-1.5"><FileText className="h-4 w-4 text-[#8C0C14]" /> {pkg.totalQuestions} Soal Ujian</span>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-lg space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  ...
                  <span className="text-[#8C0C14] font-extrabold text-xl">
              ...
              <button
                ...
                className="w-full py-3 bg-[#8C0C14] hover:bg-[#6D080E] text-white rounded-xl font-bold text-sm shadow-md hover:shadow-lg transition-all active:scale-[0.98] border-0 cursor-pointer"
              >
